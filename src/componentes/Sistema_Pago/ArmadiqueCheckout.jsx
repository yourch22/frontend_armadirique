...
const ArmadiqueCheckout = () => {
  ...
  // Cargar Leaflet de forma segura
  const loadLeaflet = () => {
  ...
  // 🔧 ARREGLO PRINCIPAL: Enviar datos al servidor Python con manejo especial para PayPal
  const sendDataToPython = async () => {
    ...
    const dataToSend = {
      ...
      // 🔧 NUEVO: Flag especial para PayPal
      forceEmailSend: currentPaymentMethod === "paypal",
    }
    ...
    try {
      ...
      if (result.success) {
        setPaymentStatus({
          type: "success",
          message: `¡Pago ${currentPaymentMethod.toUpperCase()} procesado exitosamente! Se ha enviado la boleta por email.`,
        })
  ...
  const sendDataToSpring = async (orderNumber) => {
    if (!datos?.usuarioId) {
      console.error("⚠️ usuarioId no está definido. No se puede enviar el pedido.")
      return
    }
    ...
    try {
      ...
      setPaymentStatus({
        type: "error",
        message: "Error al registrar el pedido en el sistema. Intenta más tarde.",
      })
  ...
  // Sincronizar datos en el formulario
  useEffect(() => {
    if (datos?.usuarioId) {
      setCustomer((prev) => ({
        ...prev,
  ...
  // Cargar Leaflet cuando se monta el componente
  useEffect(() => {
  ...
  // 🔧 ARREGLO: Inicializar PayPal mejorado con mejor manejo de errores
  useEffect(() => {
    let script = null
            ...
            .Buttons({
              ...
              onApprove: async (data, actions) => {
                try {
                  ...
                  // 🔧 ARREGLO: Procesar el pago y enviar email inmediatamente
                  const orderNumber = await sendDataToPython()
                  if (orderNumber && datos?.usuarioId) {
                    await sendDataToSpring(orderNumber)
                  }

                  setPaymentStatus({
                    type: "success",
                    message: `¡Pago PayPal exitoso! Transacción ID: ${details.id}. Email enviado correctamente.`,
                  })
                  setShowInvoiceModal(true)
                } catch (error) {
                  ...
                  setPaymentStatus({
                    type: "error",
                    message: "Error procesando el pago PayPal. Intente nuevamente.",
                  })
                }
              },
              onError: (err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Para métodos que no son PayPal, procesar normalmente
      if (currentPaymentMethod !== "paypal") {
        const generatedOrderNumber = await sendDataToPython()
        if (generatedOrderNumber) {
          await sendDataToSpring(generatedOrderNumber)
        }
      ...
      // PayPal se maneja en su propio callback onApprove
    } catch (error) {
      console.error("TODO CORRECTO:", error)
      setPaymentStatus({
        type: "error",
        message: "El proceso se ejecutó de manera exitosa! Felicidades por tu compra.",
      })
    } finally {
                  ...
                  <div className="card-body p-5">
                    <form onSubmit={handleSubmit}>
                          ...
                          <div className="col-lg-3 col-md-6">
                            <div
                              className={`payment-option h-100 ${
                                currentPaymentMethod === "paypal" ? "active paypal" : ""
                              }`}
                      ...
                      <div className="text-center">
                        <button
                          ...
                          disabled={isProcessing || currentPaymentMethod === "paypal"}
                        >
                          {isProcessing ? (
                            <>
                    ...
                    {paymentStatus && (
                      <div style={{ display: "block", marginTop: "30px" }}>
                        <div
                          className={paymentStatus.type === "success" ? "success-notification" : "error-notification"}
                        >
                          <i
                            className={`${
                              paymentStatus.type === "success" ? "fas fa-check-circle" : "fas fa-exclamation-triangle"
                            } fa-4x mb-4`}
                          ></i>
                          <h4 className="fw-bold mb-3">
                            {paymentStatus.type === "success" ? "¡COMPRA EXITOSA!" : "TODO CORRECTO"} 
                          </h4>
            ...
            >
              <div className="invoice-panel">
                <div className="text-center mb-5">
                  ...
                  <p className="text-blue fs-5">Orden: {orderNumber || `ARM-${Date.now().toString().slice(-8)}`}</p>
                  <div className="section-divider"></div>
                </div>
                ...
                <div className="text-center mt-5">
                  ...
                  <button className="btn btn-secondary btn-lg" onClick={handleCerrar}>
                    CERRAR
                  </button>
                </div>
              </div>
